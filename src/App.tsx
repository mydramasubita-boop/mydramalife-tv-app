...
const MyDramaApp = () => {
  ...
  const addToHistory = (project, episodeIndex = 0) => {
    const newHistory = [
  ...
  const removeFromHistory = (projectId) => {
    const newHistory = history.filter(h => h.projectId !== projectId);
    setHistory(newHistory);
    localStorage.setItem('mydrama_history', JSON.stringify(newHistory));
  };

  const playVideo = (project, episodeIndex = 0) => {
    setPlaying(project);
  ...
  const getFilteredProjects = () => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(p =>
        ...
        p.generi.some(g => g.toLowerCase().includes(searchQuery.toLowerCase())) ||
        p.attori.some(a => a.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
        ...
        <video
          ...
          onTimeUpdate={(e) => {
            ...
            if (timeLeft <= 0.75 && timeLeft > 0) {
              video.style.opacity = timeLeft / 0.75;
            }
          }}
    ...
    return (
      ...
      >
        ...
        <div style={{
          ...
          {showNextButton && playing.video_data.is_serie && currentEpisode < playing.video_data.episodi.length - 1 && (
            <div style={{
              position: 'absolute',
              bottom: '140px',
              right: '40px',
          ...
          <div style={{ display: 'flex', gap: '60px', marginBottom: '60px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: '400px' }}>
              ...
              <div style={{ display: 'flex', gap: '20px', marginBottom: '30px', flexWrap: 'wrap' }}>
                {selectedProject.generi.map((genere, i) => (
                  <span
                    key={i}
                    onClick={() => {
              ...
              <div style={{ marginBottom: '30px' }}>
                ...
                <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                  {selectedProject.attori.map((attore, i) => (
                    <span
                      key={i}
                      onClick={() => {
